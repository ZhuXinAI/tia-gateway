...
const defaultComponents = memoizeMarkdownComponents({
  h1: ({ className, ...props }) => (
    <h1
      className={cn('aui-md-h1 mb-2 scroll-m-20 text-base font-semibold first:mt-0 last:mb-0', className)}
      {...props}
    />
  ),
  h2: ({ className, ...props }) => (
    <h2
      className={cn('aui-md-h2 mt-3 mb-1.5 scroll-m-20 text-sm font-semibold first:mt-0 last:mb-0', className)}
      {...props}
    />
  ),
  h3: ({ className, ...props }) => (
    <h3
      className={cn('aui-md-h3 mt-2.5 mb-1 scroll-m-20 text-sm font-semibold first:mt-0 last:mb-0', className)}
      {...props}
    />
  ),
  h4: ({ className, ...props }) => (
  ...
  blockquote: ({ className, ...props }) => (
    <blockquote
      className={cn('aui-md-blockquote my-2.5 border-l-2 border-muted-foreground/30 pl-3 text-muted-foreground italic', className)}
      {...props}
    />
  ),
  ul: ({ className, ...props }) => (
  ...
  table: ({ className, ...props }) => (
    <table
      className={cn('aui-md-table my-2 w-full overflow-y-auto border-separate border-spacing-0', className)}
      {...props}
    />
  ),
  th: ({ className, ...props }) => (
  ...
  td: ({ className, ...props }) => (
    <td
      className={cn('aui-md-td border-b border-l border-muted-foreground/20 px-2 py-1 text-left last:border-r [[align=center]]:text-center [[align=right]]:text-right', className)}
      {...props}
    />
  ),
  tr: ({ className, ...props }) => (
  ...
  pre: ({ className, ...props }) => (
    <pre
      className={cn('aui-md-pre overflow-x-auto rounded-b-lg rounded-t-none border border-border/50 border-t-0 bg-muted/30 p-3 text-xs leading-relaxed', className)}
      {...props}
    />
  ),
