...
export class LarkChannel extends AbstractChannel {
  ...
  constructor(options: LarkChannelOptions) {
    ...
    this.logger = options.logger.child(`lark:${options.id}`)
  }
  ...
  async start(): Promise<void> {
    ...
    this.eventDispatcher.register({
      'im.message.receive_v1': async (event: unknown) => {
        ...
        if (!message || isStaleMessage(message.timestamp)) {
          return
        }

        void this.emitMessage(message).catch((error) => {
          this.logger.error(`Failed to process inbound message ${message.id}`, error)
        })
      }
    })
  ...
  private async loadBotMentionIds(): Promise<void> {
    ...
    try {
      ...
      if (response.code !== 0) {
        this.logger.warn(
          `Failed to fetch bot info for mention detection: ${response.msg ?? 'unknown error'}`
        )
        return
      }
      ...
      this.logger.warn('Failed to fetch bot info for mention detection', error)
    }
  }
}
